...
size_t counter(const char *str, size_t count)
{
    if (str == NULL)
        return 0;
    if (str[count] == '\0')
...
size_t CountCharinString(const char *str, const char *containsChar)
{
...
{
    const size_t strLen = getStringLength(str);
    if (strLen == 0)
    {
        return NULL;
    }
